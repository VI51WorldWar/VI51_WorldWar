...
public class Warrior extends Ant {
    ...
    /**
     * Function doNextMovement Transform the next movement in path into the move
     * action !! WARNING !! The movement path MUST be filled when you call this
     * function
     * 
     * @param body
     */
    ...
    /**
     * Function updatePositions Update the relative position of the starting
     * point and the current position of the body
     * 
     * @param body
     */
    ...
    private boolean dropPheromoneIfNeeded() {
        ...
        Square[][][] perceivedMap = this.getBody().getPerception()
                .getPerceivedMap();
        ...
        boolean validHomePheromoneFound = false;
        ...
        for (int i = 0; i < perceivedMap.length
                && !(validHomePheromoneFound && validDangerPheromoneFound); ++i) {
            for (int j = 0; j < perceivedMap[0].length
                    && !(validHomePheromoneFound && validDangerPheromoneFound); ++j) {
                for (WorldObject wo : perceivedMap[i][j][0].getObjects()) {
                    ...
                    if (wo instanceof Pheromone) {
                        ...
                        if (p.getSide().equals(this.getBody().getSide())
                                && p.getStrength() / p.getStartingStrength() > this.acceptedOldPh
                                && Point3D.euclidianDistance(p.getPosition(),
                                        this.getBody().getPosition()) <= this.acceptedDistancePh) {
                            if (p.getMessage() == Message.HOME) {
                                validHomePheromoneFound = true;
                            } else if (p.getMessage() == Message.DANGER) {
                                validDangerPheromoneFound = true;
                            }
                        }
                    } else if (!dangerPheromoneRequired
                            && wo instanceof InsectBody) {
        ...
        if (validHomePheromoneFound && validDangerPheromoneFound) {
            return false;
        }
        ...
        // ago.
        if (!validHomePheromoneFound
                && this.relativeStartingPointPosition != null) {
            getBody().setAction(
                    new DropPheromone(this.getBody().getSide(), this.getBody()
                            .getPosition(), Message.HOME,
                            (targetPosition != null) ? Direction.toDirection(
            ...
            for (int j = 0; j < perceivedMap[0].length; ++j) {
                for (WorldObject wo : perceivedMap[i][j][0].getObjects()) {
                    ...
                    if (wo.getTexturePath().equals(
                        ...
                        System.out
                                .println("Error in goHome() for Warrior l.248 reached"); //$NON-NLS-1$
                        return;
                        ...
                        System.out
                                .println("Error in goHome() for Warrior l.299 reached"); //$NON-NLS-1$
                        return;
                    }
                }
            }
        ...
        if (closestEnemyBody == null) {
            ...
            if (this.relativeStartingPointPosition != null
                    && Point3D.euclidianDistance(new Point3D(0, 0, 0),
                            this.relativeStartingPointPosition) > 20) {
                // Return to home
                goHome();
            }

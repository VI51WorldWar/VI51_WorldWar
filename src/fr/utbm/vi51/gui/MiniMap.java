...
public class MiniMap extends JPanel {
    ...
    // !!!!! DO NOT MODIFY HERE !!!!!!!!
    private Rectangle rCurrentView;
    // Reference to the parent Window
    private Window rParent;

    private JButton butLevelUp;
    private JButton butLevelDown;
    ...
    private void updateZoom() {
        this.currentZoom = Math.min(this.getHeight()
                / Environment.getInstance().getMapHeight(), this.getWidth()
                / Environment.getInstance().getMapWidth());
        // Update the Width and height of each tile
        this.currentTileWidth = this.mapView.width
                / Environment.getInstance().getMapWidth() * this.currentZoom;
        this.currentTileHeight = this.mapView.height
                / Environment.getInstance().getMapHeight() * this.currentZoom;
    }
    ...
    void moveView(Point coordViewCenter) {
        ...
        Point squareCoord = new Point(
                coordViewCenter.x / this.currentTileWidth, coordViewCenter.y
    ...
    private void drawViewRect(Graphics g) {
        g.drawRect((this.mapView.x + this.rCurrentView.x)
                * this.currentTileWidth, (this.mapView.y + this.rCurrentView.y)
                * this.currentTileHeight, (this.rCurrentView.width)
                * this.currentTileWidth, (this.rCurrentView.height)
                * this.currentTileHeight);
    }
            ...
            for (int j = 0; j < mapHeight; j++) {
                ...
                g.drawImage(
                        ImageManager.getInstance().getImage(
                                currentSquare.getLandType().getTexturePath()),
                        (int) drawPosition.x, (int) drawPosition.y,
                        this.currentTileWidth, this.currentTileHeight, this);

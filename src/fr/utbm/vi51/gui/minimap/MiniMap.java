...
/**
 * @author behnnes This class represents the mini map in the user interface Use
 *         the moveTo function to move the mini map Use the setDimensions
 *         function to set minimap's height and width !!!! WARNING !!!! A
 *         dimension cannot be smaller than the number of square in this
 *         dimension Use the paint function to paint the mini map on a Graphics
 */
public class MiniMap {
    private Point2d position = new Point2d(0, 300);
    private int height;
    private int width;
    ...
    public void paint(Graphics g, JPanel panel, Rectangle viewRect) {
            ...
            for (int j = 0; j < displayedTilesY; j++) {
                ...
                if (!objs.isEmpty()) {
                    ...
                    g.drawImage(
                            ...
                            tileWidth, tileHeight, panel);
                } else {
                    // Draw land
                    g.drawImage(
                            ImageManager.getInstance().getImage(
                                    currentSquare.getLandType()
                                            .getTexturePath()),
                            (int) drawPosition.x, (int) drawPosition.y,
                            tileWidth, tileHeight, panel);
                }
        ...
        g.drawRect((int) position.x + viewRect.x,
                (int) position.y + viewRect.y, viewRect.width * tileWidth,
